...
function Navbar() {
  ...
  return (
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex-1 flex justify-between items-center pl-7 hidden lg:flex">
              ...
              <div className="flex items-center group cursor-pointer">
                <IoLocationSharp size={25} className="group-hover:text-[#FFC65C]" />
                <span className="ml-1 group-hover:text-[#FFC65C] text-md">Find us</span>
              </div>
              ...
              <div className="flex items-center">
                <ul className="flex gap-9">
                  ...
                  <li className="flex items-center gap-2 group cursor-pointer">
                    <BsFillTelephoneInboundFill size={18} className="group-hover:text-[#FFC65C]" />
    ...
    {isMobileMenuOpen && (
        <div className="lg:hidden bg-[#173366] absolute top-0 w-full  text-white py-4 px-7">
            ...
            {mainNavItems.map((item, index) => (
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  {typeof item === "object" && (
                    <IoIosArrowDown className={`transition-transform duration-300 ${activeDropdown === index ? "rotate-180" : ""}`} />
                  )}
                </div>
                ...
                {typeof item === "object" && item.title === "Treatments" && activeDropdown === index && (
                  <div className="flex flex-col pl-4 mt-2">
                    {item.dropdownItems.map((dropdownItem, dropIndex) => (
                      <div key={dropIndex} className="py-2 hover:bg-blue-50 hover:text-[#1a237e] cursor-pointer">
                        {dropdownItem}
                      </div>
                    ))}
                  </div>
                        ...
                        {activeState === state.name && (
                          <div className="pl-4">
                            {state.branches.map((branch, branchIndex) => (
                              <div
                                key={branchIndex}
                                className="py-2 hover:bg-blue-50 hover:text-[#1a237e] cursor-pointer"
                              >
                                {branch}
                              </div>
                            ))}
